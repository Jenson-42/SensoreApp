...
namespace SensoreApp.Services
{
    public class FrameMetricsService
    {
        ...
        /// <summary>
        /// Generates sample 32x32 frame data for testing
        /// Simulates realistic pressure patterns based on 12-bit sensor (0-4095)
        /// </summary>
        public int[] GenerateSampleFrameData(string pattern = "sitting")
        {
            ...
            {
                case "sitting":
                    // Simulate sitting: high pressure in lower-right quadrant (matches real data)
                    for (int row = 0; row < 32; row++)
                    {
                        for (int col = 0; col < 32; col++)
                        {
                            ...
                            // High pressure region (lower-right - typical sitting pattern)
                            if (row >= 12 && row <= 28 && col >= 12 && col <= 28)
                            {
                                // Buttocks area - highest pressure
                                if (row >= 16 && row <= 24 && col >= 16 && col <= 24)
                                {
                                    frameData[idx] = random.Next(400, 600); // High pressure
                                }
                                // Surrounding contact area
                                else
                                {
                                    frameData[idx] = random.Next(50, 300); // Medium pressure
                                }
                            }
                            // No contact area (most of upper-left)
                            else
                            {
                                frameData[idx] = random.Next(0, 20); // Minimal/no pressure
                            }
                        }
                    }
                    break;
                ...
                case "concentrated":
                    // High pressure in one spot (pressure ulcer risk!)
                    for (int i = 0; i < 1024; i++)
                    {
                        frameData[i] = random.Next(0, 30);
                    }
                    // Add concentrated pressure region
                    for (int row = 14; row <= 18; row++)
                    {
                        for (int col = 14; col <= 18; col++)
                        {
                            int idx = row * 32 + col;
                            frameData[idx] = random.Next(800, 1200); // Very high concentrated pressure
                        }
                    }
                    break;

                case "even":
                    // Evenly distributed pressure (ideal scenario)
                    for (int row = 0; row < 32; row++)
                    {
                        for (int col = 0; col < 32; col++)
                        {
                            int idx = row * 32 + col;
                            if (row >= 8 && row <= 24 && col >= 6 && col <= 26)
                            {
                                frameData[idx] = random.Next(200, 350); // Moderate, even pressure
                            }
                            else
                            {
                                frameData[idx] = random.Next(0, 20);
                            }
                        }
                    }
                    break;
